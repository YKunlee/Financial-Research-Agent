...
### 2) 公司识别（T1）
- `src/finresearch_agent/identify/resolver.py`  
  从 `data/companies.csv` + `data/aliases.json` 解析查询，输出 `symbol/market/company_name`。
  不使用 LLM 猜测。

### 3) 缓存（T2/T3/T6）
- `src/finresearch_agent/cache/redis_cache.py`  
  Redis JSON 缓存封装，所有数据先查缓存再查外部。
- `src/finresearch_agent/cache/memory_cache.py`  
  测试/本地替代缓存实现。
...
### 4) 数据源（T2/T3/T6）
- `src/finresearch_agent/datasources/market.py`  
  默认 Stooq 行情日线，支持缓存与回源。
- `src/finresearch_agent/datasources/financials.py`  
  Alpha Vantage 财报（需 `ALPHAVANTAGE_API_KEY`）。
- `src/finresearch_agent/datasources/news.py`  
  NewsAPI 新闻（需 `NEWSAPI_KEY`）。

### 5) 指标与风险（T4/T5）
- `src/finresearch_agent/metrics/technical.py`  
  技术指标（MA、波动率、最大回撤）。
- `src/finresearch_agent/metrics/risk.py`  
  风险指标（Sharpe、历史 VaR）。

### 6) 规则引擎（T7）
- `src/finresearch_agent/rules/ruleset_v1.py`  
  风险规则定义（阈值与比较算子）。
- `src/finresearch_agent/rules/engine.py`  
  规则执行，输出结构化风险标记。

### 7) 快照（T8）
- `src/finresearch_agent/snapshot/builder.py`  
  汇总输出，生成不可变 `analysis_id`，写入 `snapshots/`。

### 8) LLM 解释（T9）
- `src/finresearch_agent/llm/explainer.py`  
  LLM 只读 JSON 快照；校验“无新增数字”与“必须引用版本号/analysis_id”。
  无 `OPENAI_API_KEY` 时走本地确定性解释。

### 9) 对外接口（T10）
- `src/finresearch_agent/agent/stock_agent.py`  
